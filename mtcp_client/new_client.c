...
int send_request(struct thread_context*, int, int);
int receive_request(struct thread_context*, int, int);
    ...
    while(!done) {
        ...
        for(int i = 0; i < event_count; i++) {
            ...
            if(ev.events & MTCP_EPOLLERR) {
                ...
                send_request(ctx, epoll_id, ev.data.sockid);
            } else if(ev.events & MTCP_EPOLLIN) {
                receive_request(ctx, epoll_id, ev.data.sockid);

                mtcp_close(ctx->mctx, ev.data.sockid);
            }
        }
        usleep(SLEEP_TIME);
    }
...
int send_request(struct thread_context* ctx, int epoll_id, int socket_fd) {
    char hello[BUFFER_SIZE];
...
int receive_request(struct thread_context* ctx, int epoll_id, int socket_fd) {
    char hello_receive[BUFFER_SIZE];
